...
export default function BottomMenu({openCv, openPortfolio, openAboutMe, openNotification, openAboutSystem}) {
  const HandleRefresh = () => {
  ...
  return (
    <div className="flex flex-row bg-customGrey fixed bottom-[52px] left-[2px] h-[338px] w-[250px] cardAndTableBorder box-shadow">
      ...
      <MouseClick className={'w-full'}>
        ...
        <MouseClick className={'w-full'}>
          <button className="flex flex-row gap-2 p-2 w-full items-center text-black hover:bg-customGrey25" onClick={openAboutSystem}>
            <img src="/system.png" alt="SystemInfo" />About website
          </button>
        </MouseClick>
        <MouseClick className={'w-full'}>
        <button
